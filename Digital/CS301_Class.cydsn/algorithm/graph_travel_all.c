...
static Vector* make_acyclic(const Graph* graph, graph_size_t start) {
    size_t i;
    node_orders = vector_create(graph->nodes->size);
    ...
    for (i = 0; i < graph->unique_edges; i++) {
        vector_append(edge_priority, UVOID_T_MAX);
    }
    ...
    while(frontier->size > 0) {
        ...
        for (i = 0; i < current_node->edges->size; i++) {
            ...
            if (came_from_edge == NULL) {
                ...
                linked_list_push(frontier, arc->destination);
                break;
            }
            else {
                if (current_node_id != start &&
                    (GraphNode*) vector_get(came_from, current_node_id) != current_edge &&
                    (arc->destination != start || came_from_edge != current_edge) {
                        current_node_order = vector_get(node_orders, current_node_id);
                        ...
                        graph_edge_remove(current_edge);
                }
            }
            ...
            if (i == current_node->edges->size - 1) {
                ...
                if (came_from_edge != NULL) {
                    vector_set(edge_priority, came_from_edge->id, (void) branch_length)
                }
            }
        }
    }
...
static LinkedList* make_path(const Graph* graph, graph_size_t start, Vector* edge_priority) {
    size_t i;
    LinkedList* path = linked_list_create();
    LinkedList* frontier = linked_list_create();
    Vector* came_from = vector_create(graph->nodes->size);
    for (i = 0; i < graph->nodes->size; i++) {
        vector_append(came_from, UVOID_T_MAX);
    }
    ...
    vector_set(came_from, start, start);

    while(frontier->size > 0) {
        current_node_id = (uvoid_t) linked_list_peek_stack(frontier);
        
        linked_list_add(path, (uvoid_t) current_node_id);
        ...
        for (i = 0; i < current_node->edges->size; i++) {
            current_edge = vector_get(current_node->edges, i);
            current_priority = vector_get(edge_priority, current_edge->id);
            priority_queue_add(priority, current_priority, current_edge);
        }
        ...
        while (!priority_queue_empty(priority)) {
            ...
            came_from_node_id = vector_get(came_from, arc->destination)
            if (came_from_node_id == UVOID_T_MAX) {
                vector_set(came_from, arc->destination, current_node_id);
